# BCN-AICX-01 — Integração Central AI Chat (BEACON)

**Owner**: backend (rewire-beacon)
...
Para participar do fluxo, **BEACON** precisa expor o endpoint
canonical `POST /agent/v1/invoke` aderente a [[INTER_AGENT_COMM_SPEC]]
mapeando para as capabilities já declaradas via BCN-CAP-01 em
`phase-9-capability-registry/`.

**Purpose do serviço (recap)**: Status page + uptime monitoring multi-region (futuramente NLP intent multi-canal).

**BEACON tem IA PLANEJADA** (vide [[AI_INVENTORY]] §Matrix `planned`). Implementar sub-agent **mínimo template-based** seguindo `services/rewire-app/apps/chat-orchestrator/docs/templates/minimal-subagent.md` (criado em ticket APP follow-up `APP-AICX-02`). Cada capability_id mapeia para operação determinística existente do serviço + opcional LLM call via NOVA Gateway para gerar resumo PT-BR. ADR de runtime IA completo fica fora do escopo deste ticket (vide AI_INVENTORY §Gap action plan).
...
```json
{
  "capability": "rewire.beacon.<capability_name>",
  "input": {},
  "metadata": {
    "deadline_ms": 30000,
    "max_cost_usd": 0.05,
...
9. Anchor response no CITADEL — propagar `audit_chain_hash` na
   response.
10. Emitir audit-trail event canonical `rewire.beacon.<capability>.invoked`
...
Para **BEACON**, capabilities iniciais a mapear (alinhar com
`capabilities.yaml` que CAP-01 cria):

- `rewire.beacon.list_incidents`
- `rewire.beacon.create_incident`
- `rewire.beacon.attach_status_page`
...
- [ ] Audit-trail event `rewire.beacon.<capability>.invoked`
...
- BCN-CAP-01 (já criado) — REST registry baseline expondo `capabilities.yaml`

## Notas

- Não exige refactor de qualquer feature atual do **BEACON**.
