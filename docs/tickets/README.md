...
| [[BCN-151]] | Wiring `BeaconOverview.tsx` → `/v1/overview` | [x] | frontend |
| [[BCN-152]] | Wiring `BeaconMessages.tsx` → `/v1/messages` (list + filter) | [x] | frontend |
| [[BCN-153]] | Wiring `BeaconTemplates.tsx` → `/v1/templates/*` | [x] | frontend |
| [[BCN-154]] | Wiring `BeaconJourneys.tsx` → `/v1/journeys/*` | [x] | frontend |
| [[BCN-155]] | Wiring `BeaconSuppression.tsx` → `/v1/suppression` | [x] | frontend |
| [[BCN-156]] | Wiring `BeaconDomains.tsx` → `/v1/domains` | [x] | frontend |
| [[BCN-157]] | Wiring `BeaconSmsNumbers.tsx` → `/v1/sms-numbers` | [x] | frontend |
| [[BCN-158]] | Wiring `BeaconWhatsapp.tsx` → `/v1/whatsapp` | [x] | frontend |
| [[BCN-159]] | Wiring `BeaconPushApps.tsx` → `/v1/push-apps` | [x] | frontend |
| [[BCN-160]] | Wiring `BeaconWebhooks.tsx` → `/v1/webhooks` | [x] | frontend |
| [[BCN-161]] | Wiring `BeaconAnalytics.tsx` → `/v1/analytics/messages` | [x] | frontend |
| [[BCN-162]] | Wiring `BeaconApiKeys.tsx` → `/v1/api-tokens` | [x] | frontend |
| [[BCN-163]] | Wiring `BeaconLgpd.tsx` → `/v1/audit/lgpd/dsar` | [x] | frontend |
| [[BCN-164]] | Wiring `BeaconBilling.tsx` → `/v1/billing/*` | [x] | frontend |
| [[BCN-165]] | Wiring `BeaconChain.tsx` (visualização audit chain BLAKE3) | [x] | frontend |
| [[BCN-166]] | Wiring `BeaconDeliverability.tsx` (Postal reputation) | [x] | frontend |
